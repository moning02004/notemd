...
export default function Page() {
    ...
    const changeEditPermission = () => {
        ...
        if (tag) console.log("수정권한 update api to ")
    }
    ...
    const handleContent = (values: { title: string; content: unknown }) => {
          ...
          ${isOpenedSetting ? "translate-x-0" : "translate-x-full"}`}>
                    <div className="flex flex-col p-5 w-full">
                        ...
                        <div className="min-h-[7rem]">
                            ...
                            {isPublic && <input type="text" value={url}
                                                className="border w-full p-2 mt-1 rounded border-[#ededed] outline-none bg-gray-100"
                                                readOnly/>}
                        </div>

                        <div className="flex flex-col">
                            <span>편집 권한</span>
                            <select className="p-2 border rounded mt-1 border-[#ededed] bg-[#fff6f6]"
                                    ref={editPermissionRef} onChange={changeEditPermission}>
