...
import { FiExternalLink, FiHome, FiLogOut, FiPenTool, FiPlus, FiUser, FiUsers, FiUserX } from 'react-icons/fi';

const iconSize = 14
const sideMenu = [
  {title: "대시보드", icon: <FiHome  size={iconSize} />, url: "/dashboard"},
  {title: "노트", icon: <FiPenTool  size={iconSize} />, url: "/notes", rightIcon: <FiPlus size={iconSize} />, rightUrl: '/notes/add'},
          ...
          sideMenu.map((el, index) => {
            return (
              ...
              >
                ...
                {
                  el.rightIcon && (<Link className='flex justify-center m-auto hover:rounded hover:bg-[#adadad] p-1' href={el.rightUrl}>
                    {el.rightIcon}
                  </Link>)
                }
              </div>
